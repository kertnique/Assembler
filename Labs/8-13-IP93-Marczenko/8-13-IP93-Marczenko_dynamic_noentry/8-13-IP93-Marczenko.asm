...
INCLUDELIB \masm32\lib\kernel32.lib     ; for ExitProcess, LoadLibrary, GetProcAddress, FreeLibrary
...
calc MACRO i, buff
    ...
    ; аргументи в зворотньому порядку
    forPushing results[i*8]
    forPushing res6[i*8]
    forPushing res5[i*8]
    forPushing res4[i*8]
    forPushing res3[i*8]
...
.data
    ...
    library_                 DB "8-13-IP93-Marczenko-lib",0
    func_                    DB "enumerate",0
...
.code
    start:
        invoke LoadLibrary, addr library_
        mov dummy1, eax
        invoke GetProcAddress, dummy1, addr func_
        mov enumerate, eax
        mov esi, 0
